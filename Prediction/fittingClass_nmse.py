...
class FittingOptimizerNMSE:
    ...
    def equation_error(self, c, equation, data, λ=0.1, L=30, ε=1e-8):
        """
        带复杂度惩罚的误差函数：
        fitness = 1 / (1 + NMSE) + λ * exp(-complexity / L)
        loss = 1 / fitness
        """
        ...
        try:
            y_pred = eval(equation, {
                'c': c, 'np': np, 'sqrt': np.sqrt, 'cbrt': np.cbrt,
                'log': np.log, 'exp': np.exp,
                **{f'x{i+1}': x[:, i].reshape(-1) for i in range(num_indep_vars)}
            })
        except Exception:
            return float('inf')  # 出错就给个很大的惩罚

        # 1. 计算 MSE 和 NMSE（归一化误差）
        mse = np.mean((y_pred - y) ** 2)
        nmse = mse / (np.sum(y ** 2) + ε)

        # 2. 计算复杂度（符号个数）
        complexity = calculate_complexity(equation)

        # 3. 惩罚形式
        fitness = 1 / (1 + nmse) + λ * np.exp(-complexity / L)

        return 1 / fitness  # 最小化这个目标函数
