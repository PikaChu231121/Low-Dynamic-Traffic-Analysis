...
class BaseAlgorithmModule:
    ...
    def scheduleOffloading(self, env: AirFogSimEnv):
        ...
        for task_dict in all_task_infos:
            ...
            # 首先检查任务依赖
            dependency_status = env.task_manager.checkTaskDependency(task_node_id, task_id)
            if dependency_status is None:
                # (f"任务 {task_id} 有父任务失败，跳过卸载")
                failed_offloads += 1
                continue
            elif dependency_status is False:
                # print(f"任务 {task_id} 依赖未满足，暂不卸载")
                failed_offloads += 1
                continue
            ...
            if not neighbor_infos:
                # print(f"无法为节点 {task_node_id} 的任务 {task_id} 找到邻居节点")
                failed_offloads += 1
                continue
            ...
            for neighbor in neighbor_infos:
                ...
                if flag:
                    # print(f"成功将任务 {task_id} 从 {task_node_id} 卸载到 {target_node_id}")
                    offload_success = True
                    successful_offloads += 1
                    break
                else:
                    # 详细记录该目标节点失败的原因
                    # print(f"无法将任务 {task_id} 从 {task_node_id} 卸载到 {target_node_id}")
                    
                    # 检查任务是否在等待卸载列表中
                    is_in_waiting_list = False
                    if task_node_id in env.task_manager._waiting_to_offload_tasks:
                        for task_info in env.task_manager._waiting_to_offload_tasks[task_node_id]:
                            if task_info.getTaskId() == task_id:
                                is_in_waiting_list = True
                                break
                    
                    if not is_in_waiting_list:
                        print(f"  原因：任务 {task_id} 不在节点 {task_node_id} 的等待卸载列表中")
            
            if not offload_success:
                failed_offloads += 1
                print(f"警告: 尝试所有邻居节点后，仍无法卸载任务 {task_id}")
        
        # 输出统计信息
        # print(f"任务卸载统计: 成功 {successful_offloads}, 失败 {failed_offloads}")
