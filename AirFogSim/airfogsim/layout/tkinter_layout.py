...
class TkinterLayout(tk.Tk, BaseLayout):
    def __init__(self, config, env):
        ...
        # -------------draw the map----------------
        self.nonfly_zones = config['traffic']['nonfly_zone_coordinates']
        self.canvas_nonfly_zones = []  # 存储禁飞区的Canvas对象
        self.canvas.tag_raise("no_fly_zone")
    ...
    def zoom(self, event):
        ...
        self.canvas_vehicle_images = {}
        self.canvas.delete("vehicle")
        self.canvas.delete("no_fly_zone")  # 强制清除旧禁飞区
    ...
    def draw_map(self, map_data):
        ...
        # 先绘制地图
        self.map_image_data = ImageTk.PhotoImage(map_image)
        self.canvas_map_image = self.canvas.create_image(0, 0, image=self.map_image_data, anchor=tk.NW)
        # 再绘制禁飞区
        self.draw_no_fly_zones()
        return self.map_image_data
    ...
    def draw_no_fly_zones(self):
        # 清除旧禁飞区
        self.canvas.delete("no_fly_zone")
        self.canvas_nonfly_zones.clear()

        # 绘制新禁飞区
        for zone_coords in self.nonfly_zones:
            pixel_coords = []
            ...
            # 绘制半透明红色多边形
            # 使用浅红色 + 点状纹理模拟半透明效果
            zone_id = self.canvas.create_polygon(
                pixel_coords,
                fill='#ff9999',
                outline='darkred',
                stipple='gray25',
            ...
            self.canvas.tag_raise(zone_id)  # 改为提升到最上层
            self.canvas_nonfly_zones.append(zone_id)
